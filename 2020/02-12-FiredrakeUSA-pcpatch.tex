  ...
  \begin{challenge}{\texttt{PCPATCH}}
    ...
    \begin{columns}
      ...
      \begin{column}{0.53\textwidth}
        ...
        \begin{itemize}
        \item Batched BLAS?
        \item Extruded meshes?
          \begin{center}
            \begin{tikzpicture}[scale=0.75]
              \foreach \i in {0, 1, 2, 3, 4, 5} {
              \foreach \j in {0, 1, 2, 3} {
              \coordinate (\i-\j) at ($({sin(60) * 1.5 * \i}, {sin(60) * 0.5 * \j})$);
              }
              }
              \draw[blue, line join=miter, thick] (0-0) -- (5-0) -- (5-3) -- (0-3)
              -- cycle;

              \foreach \i in {1, 2, 3, 4} {
              \draw[blue, line join=miter, thick] (\i-0) -- (\i-3);
              }
              \foreach \i in {1, 2} {
              \draw[blue, line join=miter, thick] (0-\i) -- (5-\i);
              }

              \foreach \i/\j in {0/1, 1/2, 2/3, 3/4, 4/5} {
              \foreach \k in {0, 1, 2, 3} {
              \draw[solid, blue, fill=blue] ($(\i-\k)!0.5!(\j-\k)$) circle (0.05);
              }
              }
              \foreach \i/\j in {0/1, 1/2, 2/3} {
              \foreach \k in {0, 1, 2, 3, 4, 5} {
              \draw[solid, blue, fill=blue] ($(\k-\i)!0.5!(\k-\j)$) circle (0.05);
              }
              }
