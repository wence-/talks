...
\begin{poster}{
  ...
  \begin{posterbox}[name=future, column=0, below=rules,
    span=3]{Future work}
    \tcbset{center title, colframe=firedrake_blue,colback=firedrake_blue!10!white,fonttitle=\bfseries}
    \begin{tcolorbox}[title=Bernstein,left=0mm,right=0mm,top=0mm,bottom=0mm,boxsep=0.5mm, valign upper=center]
      Sum factorisation on simplex elements using Bernstein basis.

      Requires mostly similar technology, but now tensor products are ragged.
    \end{tcolorbox}
  ...
  \begin{posterbox}[name=acknowledgements,
    ...
    \begin{center}
      ...
      \includegraphics[height=2em]{nsf-logo}
    \end{center}
  \end{posterbox}
\end{poster}
\end{document}
