...
\begin{frame}
  \frametitle{Abstract mathematical framework}
  Consider, find $u \in V$ such that
  \begin{equation}
    \label{eq:bilinear}
    a(u, v) = (f, v) \text{ for all } v \in V.
  \end{equation}
  Many algorithms for solution of \cref{eq:bilinear} are induced by a space decomposition of $V$
  \begin{equation*}
    V = \sum_{i=1}^{J} V_i
  \end{equation*}
  along with an update algorithm.
\end{frame}
\begin{frame}
  \frametitle{Additive updates}
  \begin{algorithm}[H]
...
\begin{frame}
  \frametitle{Multiplicative updates}
\begin{algorithm}[H]
  \SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
  \Input{Initial guess $u_k \in V$}
  \Output{Updated guess $u_{k+1} \in V$}
...
\begin{frame}
  ...
  \begin{block}{Jacobi or Gau\ss-Seidel}
    \begin{equation*}
      V = \sum_{i=1}^{N} \operatorname{span} \{\phi_i\}
    \end{equation*}
    with $\{\phi_1, \dots, \phi_N\}$ a basis for $V$.
  \end{block}
  \begin{block}{Domain decomposition}
    \begin{equation*}
      V = V_0 + \sum_{i=1}^J V_i
    \end{equation*}
    with $V_0$ a coarse space and $V_i$ functions supported in
    $\Omega_i \subset \Omega$.
  \end{block}
  \begin{block}{Multigrid V-cycle}
    \begin{equation*}
      V = \sum_{l=L}^2 V_l + V_1 + \sum_{l=2}^L V_l
    \end{equation*}
    with $V_1 \subset V_2 \subset \dots \subset V_L = V$.
  \end{block}
\end{frame}
\begin{frame}
...
\begin{frame}
  ...
  \item Adjacency relations defined using topological queries: often
    the topological \emph{star} and \emph{closure} operations.
  \end{itemize}
  \begin{overlayarea}{\textwidth}{0.6\textheight}
    ...
    \begin{center}
      ...
      \only<2>{$\star(\text{\texttt{vertex}})$}
      \only<3>{$(\closure \circ \star)(\text{\texttt{vertex}})$}
      \only<4>{$\star(\text{\texttt{edge}})$}
      \only<5>{$(\closure \circ \star)(\text{\texttt{edge}})$}
    \end{center}
    \end{uncoverenv}
  \end{overlayarea}
\end{frame}
\begin{frame}[fragile]
 ...
 \begin{block}{Builtin}
   ...
   \item Adjacency relation that gathers points in patch
     \begin{itemize}
     \item[\texttt{star}] entities in $\star(p_i)$
     \item[\texttt{vanka}] entities in $(\closure \circ \star)(p_i)$
     \item[\texttt{pardecomp}] entities in $\Omega_i$ (local part of
       parallel mesh)
     \end{itemize}
   \end{enumerate}
 \end{block}
...
\begin{frame}
  ...
  \begin{onlyenv}<1>
    \begin{theorem}[Parameter robust parallel subspace correction]
      Consider a problem, find $u \in V$ such that
      \begin{equation*}
        a_0(u, v) + \varepsilon b(u, v) = (f, v) \text{ for all } v \in V
      \end{equation*}
      where $a_0$ symmetric positive definite and $b$ symmetric
      ...
      \end{equation*}
      the resulting subspace correction method is
      independent of $\varepsilon$ \parencite{Schoeberl:1999}.
    \end{theorem}
  \end{onlyenv}
  \begin{onlyenv}<2>
    \begin{corollary}
