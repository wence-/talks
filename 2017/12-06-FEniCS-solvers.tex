...
\end{minted}
  ...
  \begin{tikzpicture}[scale=0.55, every node/.style={transform shape},
    ...
    \node[petsc] (Jinv) {$J^{-1} \approx \mathcal{K}(J,
      J_p^{-1})$};
    \node[firedrake, below left=0.2mm of ainv.south west] (Jx) {$Jx$};
    \node[firedrake,below=0.5cm of Jx.south] (jx) {\texttt{assemble(action(J, x))}};
    \node[petsc,below right=0.5cm and -0.3cm of Jinv] (Jpinv) {$J_p^{-1} =%
      \left[%
        \begin{smallmatrix}
          A^{-1} & 0\\
          0 & S^{-1}
    ...
    \node[firedrake, below=of Ainv] (Ap)
    ...
    \node[petsc, below=1.3cm of Jpinv.south east, anchor=north] (Sinv) {$S^{-1} \approx \mathcal{K}(S,
      S_p^{-1})$};
    \node[petsc, below left=2cm and 0.6cm of Sinv, anchor=north] (S) {$Sx = (M - C A^{-1} B)x$};
