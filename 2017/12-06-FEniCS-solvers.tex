...
\begin{frame}
  ...
  \begin{tikzpicture}[
    ksp/.style={
      ...
      draw=black!50,
      font=\ttfamily},
    pc/.style={rounded rectangle,
      minimum size=6mm,
      draw=black!50,
      font=\ttfamily}]
    \node (outer) [ksp] {$P^{-1}$};
    \node (ainv.pc) [ksp, below left of=outer] {$A^{-1}$};
    \node (sinv.pc) [ksp, below right of=outer] {$S^{-1}$};
    \node (sainv.pc) [pc, below right of=sinv.pc]
    {$\hat{S}^{-1}M\hat{S}^{-1}$};
    \node (sainv.ksp) [ksp, below left of=sinv.pc] {$C A^{-1}B$};
  \end{tikzpicture}
\end{frame}
\begin{frame}[fragile]
