...
\begin{minted}[fontsize=\scriptsize]{python}
solve(a == L, x, solver_parameters={"mat_type": "matfree",
                                    "pc_type": "python",
                                    "pc_python_type": "firedrake.AssembledPC",
                                    "assembled_pc_type": "hypre"})
\end{minted}
  \begin{tikzpicture}[ufl/.style={circle,draw,black, minimum size=6mm, inner sep=2pt},
    assemble/.style={diamond,draw,black, minimum size=6mm, inner sep=0pt},
    petsc/.style={rectangle,draw,black, minimum size=6mm, inner sep=2pt},
    firedrake/.style={rounded rectangle,draw,black, minimum size=6mm, inner sep=2pt},
    arrow/.style={-stealth,line width=1}]
    \node[petsc] (ainv) {\small$A^{-1} \approx \mathcal{K}(A, A_p^{-1})$};
    \node[ufl, below left=of ainv] (Ax) {\small$Ax$};
    \node[firedrake,below=of Ax] (ax) {\scriptsize\texttt{assemble(action(a, x))}};
    \node[assemble,below right=of ainv] (ap) {\small$A_p^{-1}$};
    \node[firedrake,below=of ap] (Ap) {\small$A_p \leftarrow$\raisebox{1pt}{\scriptsize\texttt{assemble(a)}}};
    \node[petsc,below=of Ap] (Apinv) {\scriptsize\texttt{hypre($A_p$)}};
