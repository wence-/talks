...
\begin{frame}
  \frametitle{Flexible preconditioning}
  \begin{challenge}{Challenge}
    Algebraic manipulation of linear systems not sufficient to define
    preconditioners for many hard problems.
  \end{challenge}
  \begin{answer}{Exploit symbolic structure}
    Provide \emph{two-way} flow of information between
    discretisation and solver.
    \begin{itemize}
    \item PETSc for algebraic composition + manipulation of
      symbolic problem structure from Firedrake
    \item Small amount of code to write problem-specific
      preconditioner
    \item Does the right thing \emph{inside} multigrid, etc\ldots
    \item Flexible block preconditioning and monolithic multigrid smoothers.
    \end{itemize}
  \end{answer}
    \begin{flushright}
      {\scriptsize R.C.~Kirby and \textbf{L.~Mitchell}, SIAM SISC
        (2018). \arxivlink{1706.01346}{cs.MS}\nocite{Kirby:2018}

        P.E.~Farrell, M.G.~Knepley, \textbf{L.~Mitchell}, F.~Wechsung,
        ACM TOMS (2021). \arxivlink{1912.08516}{cs.MS}\nocite{Farrell:2021b}
      }
    \end{flushright}
\end{frame}
...
\begin{frame}
  ...
  \begin{challenge}{Problem}
    ...
    $\Rightarrow$ $\grad\div u$ has an enormous kernel. Need a
...
\begin{frame}
  \begin{block}{Efficient high-order FEM}
    Software and mathematics for high-performance PDE solvers on
    modern hardware. How to efficiently exploit structure for
    assembly, and solvers?

    $\Rightarrow$ EPSRC Excalibur proposal in review (with Patrick
    Farrell, Dave Moxey, Spencer Sherwin, UKAEA).
  \end{block}
  \begin{block}{Structure preserving discretisations and preconditioning}
    Need new theory to understand control of indefinite terms.
    Possibly similar to recent work on GenEO-style PCs for indefinite
    problems.

    Route to efficient solvers for fully-implicit timestepping schemes
    for numerical weather prediction (with Colin Cotter, UK Met Office).
  \end{block}
\end{frame}
